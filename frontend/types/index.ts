...
export interface InvoiceItem {
    ...
    material_code?: string;
    drg_no?: string;
    mtrl_cat?: number;    no_of_packets?: number;
    rcd_qty?: number;
    dc_dsp_qty?: number;
}
...
// ============================================================
// LINKED DOCUMENTS / CHAIN TYPES
// ============================================================

export type LinkedDocType = "PO" | "DC" | "Invoice" | "SRV" | "Contract";

export type LinkType =
    | "po_to_dc"      // PO → DC (Delivery)
    | "dc_to_invoice" // DC → Invoice (Billing)
    | "invoice_to_srv" // Invoice → SRV (Receipt)
    | "po_to_contract" // PO → Contract
    | "srv_to_contract" // SRV → Contract
    | "manual_link";    // Manual document link

export interface LinkedDocument {
    id: string;
    source_doc_type: LinkedDocType;
    source_doc_number: string;
    target_doc_type: LinkedDocType;
...
export interface DocumentLink {
    id: string;
    doc_type: LinkedDocType;
    doc_number: string;
    chain_position: number;
    chain_direction: "upstream" | "downstream" | "self";
    linked_doc: {
        doc_type: LinkedDocType;
        doc_number: string;
        doc_date?: string;
...
export interface DocumentChain {
    root_doc_type: LinkedDocType;
    root_doc_number: string;
    chain_type: LinkType;
