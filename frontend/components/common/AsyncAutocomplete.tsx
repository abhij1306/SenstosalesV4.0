...
import { Loading03Icon, Search01Icon, Cancel01Icon } from "@hugeicons/core-free-icons";
    ...
    return (
        <div ref={wrapperRef} className={cn("relative z-50", className)}>
            <div className="relative group">
                <HugeiconsIcon icon={Search01Icon} className="absolute left-3 top-2.5 h-4 w-4 text-subtle group-focus-within:text-action-primary transition-colors" />
                <Input
                    value={query}
                ...
                {!isLoading && query && (
                    <Button
                        onClick={handleClear}
                        className="absolute right-3 top-2.5 text-subtle hover:text-muted h-auto p-0 min-h-0 bg-transparent border-none shadow-none hover:bg-transparent"
                    >
