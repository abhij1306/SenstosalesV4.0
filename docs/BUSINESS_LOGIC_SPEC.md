...
SenstoSales is an ERP for a **supplier** who receives Purchase Orders from buyers/clients.
...
| **Live Chain** | Dynamic | All linked Docs (DC, SRV, Inv) are synchronized live across views. |
...
1. **PO Upload** - PO HTML contains RCD QTY. Re-uploading a PO updates `rcd_qty`.
...
The system provides live counts of linked documents to ensure data accuracy:
- **Automatic Fetching**: PO List and Detail views automatically fetch true document counts for DCs, SRVs, and Invoices from the backend.
- **Contextual Popovers**: Document details are provided via popovers that bypass regular pagination for immediate visibility.
- **Progress Consistency**: The `SupplyProgressRing` reflects the maximum progress achieved (Dispatched or Accepted) to align visual indicators with underlying document activity.
...
### PO Upload
- **Source:** HTML files from buyer portal
