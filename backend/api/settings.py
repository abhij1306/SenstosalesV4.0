...
def get_download_prefs(db: sqlite3.Connection = Depends(get_db)):
    ...
    defaults = {
        "po_html": "Downloads/PO_HTML",
        "srv_html": "Downloads/SRV_HTML",
        "challan": "Downloads/Challan",
        "invoice": "Downloads/Invoice",
        "challan_summary": "Downloads/Challan_Summary",
        "invoice_summary": "Downloads/Invoice_Summary",
        "items_summary": "Downloads/Items_Summary",
        "gc": "Downloads/GC"
    }
...
def update_download_prefs(prefs: DownloadPrefs, db: sqlite3.Connection = Depends(get_db)):
    ...
    paths = {
        ...
        "gc": prefs.gc # Added gc to paths for validation
    }
    ...
    LedgerLogger.record(
        db,
        event_class="SETTINGS_PATHS_UPDATED",
        module_path="backend.api.settings",
        entity_type="SYSTEM_CONFIG",
        entity_id="DOWNLOAD_PREFS",
        payload=paths,
        actor="USER"
    )
...
def get_supplier_info(db: sqlite3.Connection) -> dict[str, str | None]:
    """Fetch all supplier-related settings from database."""
    return {
        "supplier_name": get_setting(db, "supplier_name"),
        "supplier_gstin": get_setting(db, "supplier_gstin"),
        "supplier_address": get_setting(db, "supplier_address"),
        "supplier_contact": get_setting(db, "supplier_contact"),
        "supplier_state": get_setting(db, "supplier_state"),
        "supplier_state_code": get_setting(db, "supplier_state_code"),
        "cgst_rate": get_setting(db, "cgst_rate"),
        "sgst_rate": get_setting(db, "sgst_rate"),
        "payment_terms": get_setting(db, "payment_terms"),
    }
...
def get_settings_full(db: sqlite3.Connection = Depends(get_db)) -> dict[str, Any]:
    ...
    try:
        buyers_rows = db.execute(
            """
            SELECT 
                id, name, gstin, billing_address, shipping_address, address,
                state, state_code, place_of_supply, is_default, created_at
            FROM buyers 
            WHERE is_active = 1 
...
def update_settings_batch(settings: list[SettingsUpdate], db: sqlite3.Connection = Depends(get_db)):
    ...
    LedgerLogger.record(
        ...
        payload={"count": len(settings), "keys": [s.key for s in settings]},
        actor="USER"
    )
