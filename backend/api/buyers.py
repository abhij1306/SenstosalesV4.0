...
def create_buyer(buyer: BuyerCreate, db: sqlite3.Connection = Depends(get_db)):
    ...
    cursor = db.execute(
        """INSERT INTO buyers 
           (name, gstin, billing_address, shipping_address, address, place_of_supply, state, state_code, is_default, is_active)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (
            buyer.name,
            buyer.gstin,
            ...
            buyer.billing_address, # Sync legacy address
    ...
    LedgerLogger.record(
        db,
        event_class="BUYER_CREATED",
        module_path="backend.api.buyers",
        entity_type="BUYER",
        entity_id=str(new_id),
        payload={"name": buyer.name, "gstin": buyer.gstin},
        actor="USER"
    )
...
def update_buyer(id: int, buyer: BuyerCreate, db: sqlite3.Connection = Depends(get_db)):
    ...
    db.execute(
        """UPDATE buyers SET 
           name = ?, gstin = ?, billing_address = ?, shipping_address = ?, address = ?,
    ...
    LedgerLogger.record(
        db,
        event_class="BUYER_UPDATED",
        module_path="backend.api.buyers",
        entity_type="BUYER",
        entity_id=str(id),
        payload={"name": buyer.name, "is_active": buyer.is_active},
        actor="USER"
    )
...
def set_buyer_default(id: int, db: sqlite3.Connection = Depends(get_db)):
    ...
    LedgerLogger.record(
        db,
        event_class="BUYER_DEFAULT_CHANGED",
        module_path="backend.api.buyers",
        entity_type="BUYER",
        entity_id=str(id),
