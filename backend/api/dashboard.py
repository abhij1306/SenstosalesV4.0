...
def _fetch_recent_activity(db: sqlite3.Connection, limit: int, start_date: str | None = None) -> list[dict[str, Any]]:
    ...
    params = []
    if start_date:
        # Use po_date for consistency with summary metrics
        where_clause = " WHERE date(po.po_date) >= ?"
        params = [start_date]
    ...
    """
    po_rows = db.execute(po_query, (*params, limit)).fetchall()
    for row in po_rows:
        d = dict(row)
        d["uuid"] = f"PO_{d['number']}"
    ...
    inv_where = ""
    if start_date:
        inv_where = " WHERE date(inv.invoice_date) >= ?"
    ...
    """
    inv_rows = db.execute(inv_query, (*params, limit)).fetchall()
    for row in inv_rows:
        d = dict(row)
        d["uuid"] = f"INV_{d['number']}"
        d["party"] = d["party"] or "Client"
        d["status"] = calculate_entity_status(d["t_ord"], d["t_del"], d["t_recd"])
        activities.append(d)

    # Filter parameters for DCs
    dc_where = ""
    if start_date:
        dc_where = " WHERE date(dc.dc_date) >= ?"
    ...
    """
    dc_rows = db.execute(dc_query, (*params, limit)).fetchall()
    for row in dc_rows:
        d = dict(row)
        d["uuid"] = f"DC_{d['number']}"
        d["status"] = calculate_entity_status(d["t_ord"], d["t_del"], d["t_recd"])
        activities.append(d)

    # Filter parameters for SRVs
    srv_where = ""
    if start_date:
        srv_where = " WHERE date(s.srv_date) >= ?"

    # Recent SRVs with live status
    srv_query = f"""
        WITH srv_items_agg AS (
            SELECT srv_number, 
                   SUM(rcd_qty) as t_recd,
                   SUM(rej_qty) as t_rej
            FROM srv_items
            GROUP BY srv_number
        )
        SELECT 'SRV' as type, s.srv_number as number, s.srv_date as date, s.po_number as party, 
                0.0 as amount, s.created_at,
                COALESCE(i.t_recd, 0) as t_ord, -- For SRV, we treat received as the 'basis'
                COALESCE(i.t_recd, 0) as t_del,
                COALESCE(i.t_recd, 0) as t_recd
        FROM srvs s
        LEFT JOIN srv_items_agg i ON s.srv_number = i.srv_number
        {srv_where}
        ORDER BY s.created_at DESC LIMIT ?
    """
    srv_rows = db.execute(srv_query, (*params, limit)).fetchall()
    for row in srv_rows:
        d = dict(row)
        d["uuid"] = f"SRV_{d['number']}"
        d["party"] = f"PO #{d['party']}"
        d["status"] = "Received" # Simplified for SRV recent activity
        activities.append(d)
...
def get_dashboard_summary(time_range: str = "month", db: sqlite3.Connection = Depends(get_db)):
    ...
    try:
        ...
        # 6. Supply Chain Funnel Metrics (Filtered by respective document dates)
        # Each stage uses its own date field to show actual activity in the time range
        ...
        if start_date:
            ...
            ord_params = (start_date,)
        total_ord_qty = db.execute(ord_query, ord_params).fetchone()[0] or 0.0
        
        # Dispatched: DC date (not PO date - shows actual dispatch activity)
        dsp_query = "SELECT SUM(dci.dsp_qty) FROM delivery_challan_items dci JOIN delivery_challans dc ON dci.dc_number = dc.dc_number"
        dsp_params = ()
        if start_date:
            dsp_query += " WHERE dc.dc_date >= ?"
            dsp_params = (start_date,)
        total_dsp_qty = db.execute(dsp_query, dsp_params).fetchone()[0] or 0.0
        
        # Invoiced: Invoice date - quantity from invoice items
        inv_query = """
        ...
        inv_params = ()
        if start_date:
            inv_query += " WHERE gi.invoice_date >= ?"
            inv_params = (start_date,)
        total_inv_qty = db.execute(inv_query, inv_params).fetchone()[0] or 0.0
        
        # Received/Rejected: SRV date (not PO date - shows actual receipt activity)
        rcd_rej_query = """
        ...
        rcd_rej_params = ()
        if start_date:
            rcd_rej_query += " WHERE s.srv_date >= ?"
            rcd_rej_params = (start_date,)
        rcd_rej_row = db.execute(rcd_rej_query, rcd_rej_params).fetchone()
        ...
        # 7. Recent Activity (Consolidated) - Respects Time Range
        recent_activity = _fetch_recent_activity(db, 5, start_date)

        # 8. Performance Data (Consolidated) - Real Database Totals for Chart
        performance_data = [
            {
                "ord_qty": total_ord_qty,
                "dsp_qty": total_dsp_qty,
                "rcd_qty": total_rcd_qty,
                "rej_qty": total_rej_qty
            }
        ]

        # 9. Overdue Deliveries Count (Sync with Delivery Tracker)
        repo = DeliveryTrackerRepository(db)
        alert_stats = repo.get_alert_counts()
        overdue_count = alert_stats["overdue_count"]
        ...
        return {
            ...
            "active_challans_growth": "0%",
            "total_po_value": total_po_value,
            "po_value_growth": 0.0,
            
            "total_ord_qty": total_ord_qty,
            "total_dsp_qty": total_dsp_qty,
            "total_inv_qty": total_inv_qty,  # Actual invoiced value
            ...
            "supply_health_score": AnalyticsService.get_supply_health_score(db, start_date),
