...
def global_search(
    q: str,
    types: list[str] | None = Query(None, description="Filter by document types: po, dc, invoice, srv"),
    date_from: str | None = Query(None, description="Filter by date from (ISO format YYYY-MM-DD)"),
    date_to: str | None = Query(None, description="Filter by date to (ISO format YYYY-MM-DD)"),
    amount_min: float | None = Query(None, description="Filter by minimum amount"),
    amount_max: float | None = Query(None, description="Filter by maximum amount"),
    ...
    # Normalize types filter to lowercase for consistent matching
    allowed_types = [t.lower() for t in types] if types else ['po', 'dc', 'invoice', 'srv']

    try:
        # --------------------------------------------------------
        # 1. Search POs
        # --------------------------------------------------------
        if 'po' in allowed_types:
            po_query = """
            ...
            # Close the parenthesis for search conditions before adding filters
            po_query += ")"
            
            # Apply date filters (outside the search OR conditions)
            if date_from:
                po_query += " AND po.po_date >= ?"
                po_params.append(date_from)
            if date_to:
                po_query += " AND po.po_date <= ?"
                po_params.append(date_to)
            
            # Apply amount filters (outside the search OR conditions)
            if amount_min is not None:
                po_query += " AND po.po_value >= ?"
                po_params.append(amount_min)
            if amount_max is not None:
                po_query += " AND po.po_value <= ?"
                po_params.append(amount_max)
                ...
                results.append(
                    {
                        ...
                        "subtitle": d["date"] or "N/A",
                        "description": f"Party: {d['party'] or 'Unknown'} | Amount: ₹{d['amount'] or 0}",
        ...
        if 'dc' in allowed_types:
            ...
            # Apply date filters (outside the search OR conditions)
            if date_from:
                dc_query += " AND dc.dc_date >= ?"
                dc_params.append(date_from)
            if date_to:
                dc_query += " AND dc.dc_date <= ?"
                dc_params.append(date_to)
            ...
            for row in cursor.fetchall():
                ...
                # Fetch linked SRVs via srv_items (Verifying existence in srvs table)
                srv_rows = db.execute("""
                    SELECT DISTINCT s.srv_number FROM srv_items si
                ...
                results.append(
                    {
                        ...
                        "subtitle": d["date"] or "N/A",
                        "description": f"PO: {d['po_number']} | Party: {d['party'] or 'Unknown'}",
                        "status": calculate_entity_status(d["t_ord"], d["t_del"], d["t_recd"]),
        ...
        if 'invoice' in allowed_types:
            ...
            # Apply date filters (outside the search OR conditions)
            if date_from:
                inv_query += " AND inv.invoice_date >= ?"
            ...
            # Apply amount filters (outside the search OR conditions)
            if amount_min is not None:
                inv_query += " AND inv.total_invoice_value >= ?"
                inv_params.append(amount_min)
            if amount_max is not None:
                inv_query += " AND inv.total_invoice_value <= ?"
                inv_params.append(amount_max)
                ...
                results.append(
                    {
                        ...
                        "subtitle": d["date"] or "N/A",
                        "description": f"Party: {d['party'] or 'Client'} | Amount: ₹{d['amount'] or 0}",
                        "status": calculate_entity_status(d["t_ord"], d["t_del"], d["t_recd"]),
                        "total_qty": d["t_del"],
        ...
        # 4. Search SRVs
        # --------------------------------------------------------
        if 'srv' in allowed_types:
            srv_query = """
                WITH srv_items_agg AS (
                    SELECT srv_number, 
                           SUM(ord_qty) as t_ord,
                           SUM(accepted_qty) as t_acc, 
                ...
                SELECT s.srv_number, s.srv_date as date, s.po_number, s.invoice_number,
                       ...
                       COALESCE(v.t_val, 0) as t_val
                FROM srvs s
                LEFT JOIN srv_items_agg i ON s.srv_number = i.srv_number
                LEFT JOIN srv_value_agg v ON s.srv_number = v.srv_number
            ...
            # Apply date filters (outside the search OR conditions)
            if date_from:
                srv_query += " AND s.srv_date >= ?"
                srv_params.append(date_from)
            if date_to:
                srv_query += " AND s.srv_date <= ?"
                srv_params.append(date_to)
            
            # Apply amount filters (using calculated value from srv_value_agg)
            if amount_min is not None:
                srv_query += " AND COALESCE(v.t_val, 0) >= ?"
                srv_params.append(amount_min)
            if amount_max is not None:
                srv_query += " AND COALESCE(v.t_val, 0) <= ?"
                srv_params.append(amount_max)
                ...
                results.append(
                    {
                        ...
                        "subtitle": d["date"] or "N/A",
                        "description": f"PO: {d['po_number']} | Inv: {d['invoice_number'] or 'N/A'}",
