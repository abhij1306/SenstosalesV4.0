...
def extract_po_header(soup: BeautifulSoup) -> dict:
            ...
            for r_idx, row in enumerate(rows):
                ...
                for c_idx, cell in enumerate(cells):
                    ...
                    if found_val:
                        ...
                        if found_val.upper().startswith("DETAILS") or "PURCHASE ORDER DETAILS" in found_val.upper() or "ORDER DETAILS" in found_val.upper():
                             continue
                        ...
                        # Fix for BHEL Footer Leakage
                        if "GOVERNED BY BHEL" in found_val.upper() or "TERMS & CONDITION" in found_val.upper():
                            continue
    ...
    fields = {
        ...
        "INSPECTION AT BHEL": (r"INSPECTION\s+AT", "any", True),
...
def extract_po_items(soup: BeautifulSoup) -> list[dict]:
    ...
    for i, r in enumerate(rows):
        ...
        if "MATERIAL CODE" in texts:
            ...
            for c_i, txt in enumerate(texts):
                if "PO ITM" in txt: cols_map["PO ITM"] = c_i
                elif "MATERIAL CODE" in txt: cols_map["MATERIAL CODE"] = c_i
                elif "MTRL CAT" in txt: cols_map["MTRL CAT"] = c_i
                elif "UNIT" in txt: cols_map["UNIT"] = c_i
                elif "PO RATE" in txt: cols_map["PO RATE"] = c_i
                elif "ORD QTY" in txt: cols_map["ORD QTY"] = c_i
                elif "RCD QTY" in txt: cols_map["RCD QTY"] = c_i
                elif "REJ QTY" in txt: cols_map["REJ QTY"] = c_i
                elif "LOT NO" in txt: cols_map["LOT NO"] = c_i
                elif "DELY QTY" in txt: cols_map["DELY QTY"] = c_i
                elif "DELY DATE" in txt: cols_map["DELY DATE"] = c_i
                elif "ENTRY ALLOW DATE" in txt: cols_map["ENTRY ALLOW DATE"] = c_i
                elif "DEST CODE" in txt: cols_map["DEST CODE"] = c_i
            break
    ...
    for row in rows[header_row_idx + 1:]:
        ...
        if itm_val and itm_val.isdigit():
            ...
            # Generic Format: Description is usually in a single wide cell or 'colspan' row
            
            full_text = "".join(texts)
            if not full_text: continue # skip empty rows
            ...
            if lot_val and get_col("DELY QTY"):
                ...
                # BHEL MULTI-ITEM FIX: Some description rows mention their target item number
    ...
    for itm in items:
        ...
        if drgs:
            ...
            valid_drgs = sorted({d.strip() for d in drgs if len(d) > 4})
            if valid_drgs:
