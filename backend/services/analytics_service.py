...
class AnalyticsService:
    ...
    @staticmethod
    def get_procurement_stats(db: sqlite3.Connection, start_date: str | None = None):
        """Get aggregate statistics by material category

        Args:
            start_date: Filter PO items from this date onwards (PO date)
        """
        query = """
            SELECT
                poi.mtrl_cat,
                COUNT(DISTINCT poi.id) as item_count,
                SUM(poi.item_value) as total_value,
                SUM(poi.ord_qty) as total_ord,
                SUM(poi.rcd_qty) as total_rcd,
                SUM(poi.rej_qty) as total_rej
            FROM purchase_order_items poi
            JOIN purchase_orders po ON poi.po_number = po.po_number
            WHERE 1=1
        """
    ...
    @staticmethod
    def get_quality_stats(db: sqlite3.Connection, start_date: str | None = None):
        """Get rejection rate trends by category
        ...
        query = """
            SELECT
                ...
                SUM(poi.rej_qty) as total_rej,
                CASE
                    WHEN SUM(poi.ord_qty) > 0 THEN (SUM(poi.rej_qty) * 100.0 / SUM(poi.ord_qty))
                    ELSE 0.0
                END as rejection_rate
            FROM purchase_order_items poi
            JOIN purchase_orders po ON poi.po_number = po.po_number
