...
class DeviationService:
    ...
    def check_qty_mismatch(
        ...
        if abs(srv_ord_qty - po_ord_qty) > 0.001:
            ...
            LedgerLogger.justify(
                db,
                problem_context=f"SRV ord_qty ({srv_ord_qty}) mismatch with PO {po_number} ({po_ord_qty})",
                chosen_action="FLAG_DEVIATION",
                evidence={"po_qty": po_ord_qty, "srv_qty": srv_ord_qty, "item_no": po_item_no},
                module="backend.services.deviation",
                entity_id=srv_item_id
            )
