    ...
    try:
        with urllib.request.urlopen("http://localhost:8000/api/health", timeout=5) as response:
            if response.status == 200:
                ...
                print(f"{Colors.YELLOW}[WARN] Backend returned status {response.status}{Colors.END}")
                return False
    except urllib.error.URLError as e:
        print(f"{Colors.YELLOW}[WARN] Backend server not reachable (is it running?): {e}{Colors.END}")
        return False  # Not a code failure, just runtime state
    except Exception as e:
        print(f"{Colors.RED}[FAIL] Unexpected error checking backend: {e}{Colors.END}")
        return False
