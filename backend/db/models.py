...
class PaginatedMetadata(BaseModel):
    ...
    page: int
    limit: int
    # Filtered stats - update dynamically based on search/filter
    total_value: float | None = None
    total_taxable: float | None = None
    total_shipped: float | None = None
    total_dispatched: float | None = None
    total_rejected: float | None = None
...
# Allows alphanumeric, hyphen, underscore, and forward slash (common in PO/DC numbers)
...
class POHeader(CoreModel):
    ...
    linked_dc_count: int | None = 0
    linked_srv_count: int | None = 0
    linked_invoice_count: int | None = 0
...
class POListItem(BaseModel):
    ...
    linked_dc_count: int = 0
    linked_invoice_count: int = 0
    linked_srv_count: int = 0
    drg_no: str | None = None
