...
def validate_database_path() -> bool:
    """Validate that the database exists and is properly initialized."""
    db_path = settings.DATABASE_PATH
    
    if not db_path.exists():
        logger.error(f"Database not found at: {db_path}")
        print(f"ERROR: Database not found at: {db_path}")
        print("Please run 'python create_dummy_db.py' to create the database first.")
        return False
    
    logger.info(f"Database validated at: {db_path}")
    return True
